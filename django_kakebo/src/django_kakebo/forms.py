...
class SelectYearWeekForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field in field_types:
            self.fields[f"date_{field}"] = forms.DateField(required=False)
            self.fields[f"descr_{field}"] = forms.CharField(
                max_length=200, required=False
            )
            self.fields[f"value_{field}"] = forms.FloatField(required=False)
...
class EndOfMonthBalanceSheetForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for j in range(2):
            self.fields[f"cost_{j}_name"] = forms.CharField(
                max_length=200, required=False
            )
            for i in range(1, 6):
